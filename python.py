# -*- coding: utf-8 -*-
"""
Created on Tue Apr  5 09:52:27 2022
...
add_sidebar = st.sidebar.selectbox('All or Individual Video', ('All videos analysis','Individual Video Analysis'))
# engieer_data
df_agg_diff=df_agg.copy()
metrics_date_12=df_agg_diff['Video publish time'].max()-pd.DateOffset(months =12)
median_agg=df_agg_diff[df_agg_diff['Video publish time']>= metrics_date_12].median()
...
# get daily view data (first 25), median & percentiles 
views_days = pd.pivot_table(df_time_diff_yr,index= 'days_published',values ='Views', aggfunc = [np.mean,np.median,lambda x: np.percentile(x, 75),lambda x: np.percentile(x, 25)]).reset_index()
views_days.columns = ['days_published','mean_views','median_views','75pct_views','25pct_views']
views_days = views_days[views_days['days_published'].between(0,30)]
views_cumulative = views_days.loc[:,['days_published','median_views','75pct_views','25pct_views']] 
views_cumulative.loc[:,['median_views','75pct_views','25pct_views']] = views_cumulative.loc[:,['median_views','75pct_views','25pct_views']].cumsum()
#total picture
if add_sidebar == 'All videos analysis':
    ...
    df_agg_diff['Pulish_date']=df_agg_diff['Video publish time'].apply(lambda x:x.date())
    # df_agg_diff.columns
    df_agg_diff_final=df_agg_diff.loc[:,['Video title','Pulish_date','Views','Likes','Subscribers','Shares','Comments added','RPM(USD)','Average % viewed',
                             'Avg_duration_sec', 'Engagement_ratio','Views / sub gained']]
...
if add_sidebar == 'Individual Video Analysis':
    ...
    ag_sub_filtered=df_agg_sub[df_agg_sub['Video Title'] == video_select]
    ag_sub_filtered['Country']=ag_sub_filtered['Country Code'].apply(audience)
    ...
    fig2 = go.Figure()
    fig2.add_trace(go.Scatter(x=views_cumulative['days_published'], y=views_cumulative['25pct_views'],
                   mode='lines',
    ...
    fig2.add_trace(go.Scatter(x=views_cumulative['days_published'], y=views_cumulative['75pct_views'],
                       mode='lines', 
                       name='75th percentile', line=dict(color='royalblue', dash ='dash')))
